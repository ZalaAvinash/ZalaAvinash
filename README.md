# Hi, I'm Avinash Zala 👋

**Senior .NET & Full-Stack Engineer | AI Integration | 7+ years**

I build enterprise web apps, APIs, and ERP systems — and I'm currently adding AI/LLM capabilities to my stack. Based in Surat, India. Open to remote opportunities.

---

## 🛠️ Tech Stack

**Backend & Frameworks**
`.NET Core 8` `.NET Core 3.1/6` `ASP.NET Core` `C#` `WPF` `SignalR` `Hangfire` `Redis` `SSO / PASETO`

**Frontend**
`React.js` `TypeScript` `Angular` `JavaScript` `HTML/CSS` `Bootstrap`

**DevOps & Cloud**
`Docker` `Kubernetes` `Jenkins` `GitLab CI/CD` `Azure (Basics)` `AWS (Basics)`

**Databases**
`MSSQL` `PostgreSQL` `MySQL` `Redis`

**AI / Emerging**
`OpenAI API` `LangChain` `RAG Pipelines` `Vector Databases` `Python (learning)`

---

## 🔨 What I'm Building

| Project | Description | Stack |
|---------|-------------|-------|
| 🤖 AI Policy Assistant | RAG chatbot that answers insurance policy questions from PDFs | .NET Core, OpenAI, React |
| 📊 ERP AI Copilot | Natural language interface for ERP data queries | .NET Core, LangChain, MSSQL |
| 🧩 .NET + AI Boilerplate | Starter template for integrating OpenAI into .NET Core APIs | .NET Core 8, C# |

---

## 💼 Work Highlights

- **7+ years** delivering enterprise .NET applications
- Migrated a large **ERP system (.NET 4.5 → .NET Core)** at Aether Industries
- Built **real-time systems** with SignalR, Docker, Kubernetes & CI/CD pipelines
- Integrated **ChatGPT API** into a social platform for trending content detection
- Developed full-stack apps across insurance, ERP, social, and IoT domains

---

## 📚 Currently Learning

- 🐍 Python for AI/ML workflows
- 🔗 LangChain & LlamaIndex
- 🗄️ Vector databases (Pinecone, pgvector)
- ☁️ Azure AI-900 certification

---

## ✍️ Writing

I write about .NET + AI integration on [Hashnode](https://hashnode.com) *(coming soon)*

---

## 📬 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/avinash-zala-069086118/)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:avinashzala@outlook.com)

---

*"Building reliable systems today, adding intelligence to them tomorrow."*
